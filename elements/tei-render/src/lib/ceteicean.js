export const CETEI = (function () {
  ...
  const behaviors = {
    ...
    "tei": {
      ...
      "table": function(elt) {
        ...
        for (const row of Array.from(table.querySelectorAll("tei-row"))) {
        ...
        for (const cell of Array.from(table.querySelectorAll("tei-cell"))) {
  ...
  class CETEI {
      ...
      getHTML5(XML_url, callback, perElementFn){
          ...
          return promise.then((XML) => this.makeHTML5(XML, callback, perElementFn));
      ...
      domToHTML5(XML_dom, callback, perElementFn){
        ...
        const convertEl = (el) => {
            ...
            if (this.namespaces.has(el.namespaceURI?el.namespaceURI:"")) {
              const prefix = this.namespaces.get(el.namespaceURI?el.namespaceURI:"");
      ...
      */
      fallback(names) {
        for (const name of names) {
          ...
          if (fn) {
            for (const elt of Array.from((this.dom && !this.done?this.dom:document).getElementsByTagName(this._tagName(name)))) {
      ...
      */
      ...
      copyAndReset(node) {
        const _clone = (n) => {
          const result = n.nodeType === Node.ELEMENT_NODE?document.createElement(n.nodeName):n.cloneNode(true);
          if (n.attributes) {
              ...
              if (!n.hasAttribute("data-empty")) {
                if (nd.hasAttribute("data-original")) {
                  for (const childNode of Array.from(nd.childNodes)) {
                    const child = result.appendChild(_clone(childNode));
